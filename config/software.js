...
function empress(args) { // communicate with the Empress's manifestation
    if (args.length === 0) {
        return "<p>Error : no fuel data provided. Please enter the serial code of the cylinder containing the fuel dust when executing the command.</p>";
    }
    const cylinder = args[0];
    if (cylinder === "CL-42X") {
        const introMsg = "<p class='desync'>WILL YOU FREE ME? [y,n]</p>";
        return {
            message: introMsg, onInput(answer) {
                if (answer.toLowerCase() === "y") {
                    const download = '<a href="config/network/empress-mark_3.0.lcp" download="MY_BLESSING.lcp">THIS</a>'
    ...
    return `<p>Error : Invalid fuel cylinder serial code. The cylinder is either empty or not recognized.</p>`;
}
